...
path = "________________________________/data mining project/"

def price_csv():
    df_list = []
    
    SaveFile_Name = r'35_days_price.csv'
    file_list = os.listdir(path+'pre-processing/Price_csv')
    ...
    for i in range(0,len(file_list)):
        df = pd.read_csv(path+'pre-processing/Price_csv/' + file_list[i])
        df_list.append(df)
...
def sector_csv():
    ...
    file_list = os.listdir(path+'pre-processing/Sector_csv')
    file_list.sort(key=lambda x:int(x[4:-11]))
    
    #Loop through the names of the individual CSV files in the list and append them to the merged file
    for i in range(0,len(file_list)):
        df = pd.read_csv(path+'pre-processing/Sector_csv/' + file_list[i])
        df_list.append(df)
